...
int litehtml::render_item_block::_render(point p, const containing_block_context &containing_block_size, formatting_context* fmt_ctx, bool second_pass)
{
    ...
    // Render content
    //*****************************************
	int ret_width = _render_content(p, second_pass, self_size, fmt_ctx);
    //*****************************************
	...
	if (requires_rerender && !second_pass && !is_root())
	{
		...
		_render_content(p, true, self_size.new_width(m_pos.width), fmt_ctx);
	}
